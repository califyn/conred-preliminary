...
def site_shuffle(l, rand=None):
    sites = list(set([x[x.find("-") + 1:x.rfind("-")] for x in l]))
    random.shuffle(sites)
    sites = [[x for x in l if x[x.find("-") + 1:x.rfind("-")] == s] for s in sites]
...
class TCGADataHandler():
    def __init__(self, contrastive=['rna-seq', 'reports'], zero_shot=[], finetune=[], train_ratio=0.8, ft_train_ratio=0.5, verbose_=True, lg_types=False, seed=3019):
